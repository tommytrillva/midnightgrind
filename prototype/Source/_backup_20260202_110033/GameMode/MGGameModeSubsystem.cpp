...
void UMGGameModeSubsystem::UpdateEliminationTimer(float DeltaTime)
{
...
void UMGGameModeSubsystem::UpdateDriftScore(const FString& PlayerID, float DriftAngle, float Speed, float DeltaTime)
{
...
TArray<TPair<FString, int64>> UMGGameModeSubsystem::GetDriftLeaderboard() const
{
	TArray<TPair<FString, int64>> Leaderboard;

	for (const auto& Pair : DriftScores)
	{
		Leaderboard.Add(TPair<FString, int64>(Pair.Key, Pair.Value.TotalScore));
	}

	// Sort by score descending
	Leaderboard.Sort([](const TPair<FString, int64>& A, const TPair<FString, int64>& B)
	{
