...
/**
 * @file MGTimeOfDaySubsystem.h
 * @brief Time of Day Management Subsystem for Dynamic Day/Night Cycles
 *
 * This subsystem manages the game's time system, controlling day/night cycles,
 * lighting transitions, and time-based gameplay events in Midnight Grind.
 *
 * ## Overview
 * The Time of Day Subsystem provides:
 * - Continuous time progression with configurable speed
 * - Seamless day/night transitions with interpolated lighting
 * - Time-synchronized multiplayer support
 * - Time-based event triggers (midnight races, increased cop activity)
 * - Dynamic adjustments to traffic, pedestrians, and world atmosphere
 *
 * ## Architecture
 * This is a Game Instance Subsystem, meaning one instance persists across
 * all levels and game modes. It works in conjunction with:
 * - MGWeatherSubsystem (lighting integration)
 * - MGAmbientLifeSubsystem (traffic/pedestrian density)
 * - MGWorldEventsSubsystem (time-based events)
 *
 * ## Key Concepts
 * - **Game Time**: Internal clock in hours (0-24), independent of real time
 * - **Time Period**: Categorical division of day (Dawn, Morning, Midday, etc.)
 * - **Time Scale**: How fast game time passes relative to real time
 * - **Game Day**: Counter tracking how many in-game days have passed
 *
 * ## Usage Example
 * @code
 * // Get the time subsystem
 * UMGTimeOfDaySubsystem* TimeSystem = GetGameInstance()->GetSubsystem<UMGTimeOfDaySubsystem>();
 *
 * // Set time to midnight for a midnight race
 * TimeSystem->SetTime(0.0f);
 *
 * // Check if it's night time for bonus rewards
 * if (TimeSystem->IsNightTime())
 * {
 *     // Apply night racing bonuses
 * }
 * @endcode
 *
 * @see UMGWeatherSubsystem for weather-specific time effects
 * @see UMGWorldEventsSubsystem for time-triggered events
 */
...
/**
 * @brief Categorical time periods throughout the day
 *
 * Each period has distinct characteristics affecting gameplay:
 * - Lighting conditions (sun angle, color temperature)
 * - NPC activity levels (traffic density, pedestrian behavior)
 * - Available activities (certain races only at night)
 * - Risk levels (police activity varies by time)
 *
 * Time ranges are defined as follows (24-hour clock):
 */
UENUM(BlueprintType)
enum class EMGTimeOfDay : uint8
{
	Dawn,          ///< 5:00 - 7:00   | Golden hour sunrise, low traffic, calm atmosphere
	Morning,       ///< 7:00 - 11:00  | Full daylight, increasing traffic, business hours begin
	Midday,        ///< 11:00 - 14:00 | Peak sunlight, overhead sun, busy streets
	Afternoon,     ///< 14:00 - 17:00 | Warm lighting, peak traffic approaching
	Dusk,          ///< 17:00 - 19:00 | Golden hour sunset, rush hour traffic
	Evening,       ///< 19:00 - 22:00 | Twilight, street lights activate, nightlife begins
	Night,         ///< 22:00 - 2:00  | Full darkness, neon lights, street racing prime time
	LateNight      ///< 2:00 - 5:00   | Quietest period, reduced traffic, underground activity
};
