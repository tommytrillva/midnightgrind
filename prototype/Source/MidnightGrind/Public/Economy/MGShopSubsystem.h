...
/**
 * @file MGShopSubsystem.h
 * @brief Core shop and economy management subsystem for Midnight Grind.
 *
 * This subsystem handles all in-game commerce functionality including:
 * - Multi-currency wallet management (Cash, Gold, Reputation, Season/Crew Tokens)
 * - Shop item browsing and categorization (Vehicles, Parts, Cosmetics, etc.)
 * - Purchase transactions with validation and history tracking
 * - Daily rotating deals with automatic refresh
 * - Bundle packages with savings calculations
 * - Item ownership tracking and requirements checking
 *
 * @note This is a GameInstanceSubsystem, meaning it persists across level loads
 *       and is shared by all local players. For multiplayer, each client has
 *       their own instance that syncs with the server.
 *
 * @section usage Basic Usage Example
 * @code
 * // Get the shop subsystem
 * UMGShopSubsystem* Shop = GetGameInstance()->GetSubsystem<UMGShopSubsystem>();
 *
 * // Check if player can afford an item
 * FMGShopItem Item = Shop->GetItem(TEXT("vehicle_skyline_r34"));
 * if (Shop->CanAfford(Item.Price))
 * {
 *     EMGPurchaseResult Result = Shop->PurchaseItem(Item.ItemID);
 * }
 * @endcode
 *
 * @see UMGCurrencySubsystem For detailed currency earning mechanics
 * @see UMGInventorySubsystem For managing purchased items
 */
...
/**
 * @brief Defines the different currency types available in the game economy.
 *
 * Midnight Grind uses a multi-currency system where different activities
 * reward different currencies, and items may require specific currency types.
 */
UENUM(BlueprintType)
enum class EMGCurrencyType : uint8
{
	/** In-game cash - Primary currency earned from races and activities */
	Cash,
	/** Premium currency - Can be purchased with real money or earned rarely */
	Gold,
	/** Reputation points (cannot purchase, earned only) - Measures player standing */
	Reputation,
	/** Season tokens - Earned through seasonal battle pass progression */
	SeasonTokens,
	/** Crew tokens - Earned through crew activities and cooperative play */
	CrewTokens
};
...
// ============================================================================
// DATA STRUCTURES - Daily Deals
// ============================================================================

/**
 * @brief Represents a daily deal with a time-limited discount.
 *
 * Daily deals rotate automatically and offer significant discounts
 * on selected items to encourage daily engagement.
 */
USTRUCT(BlueprintType)
struct FMGDailyDeal
{
	...
	/** The shop item on sale */
	UPROPERTY(BlueprintReadOnly)
	FMGShopItem Item;

	/** Discount percentage (0.0 to 1.0, e.g., 0.25 = 25% off) */
	UPROPERTY(BlueprintReadOnly)
	...
	/** Time until this deal expires and is replaced */
	UPROPERTY(BlueprintReadOnly)
	FTimespan TimeRemaining;

	/** Position in the daily deals list (0 = primary deal) */
	UPROPERTY(BlueprintReadOnly)
	int32 DealIndex = 0;
};
...
/**
 * @brief Record of a completed purchase transaction.
 *
 * Transactions are logged for history display, refund processing,
...
struct FMGTransaction
{
	...
	/** Unique transaction identifier for support/refund purposes */
	UPROPERTY(BlueprintReadOnly)
	...
	/** Display name of the purchased item (for history display) */
	UPROPERTY(BlueprintReadOnly)
	FText ItemName;

	/** Currency type used for this purchase */
	UPROPERTY(BlueprintReadOnly)
	EMGCurrencyType Currency = EMGCurrencyType::Cash;
