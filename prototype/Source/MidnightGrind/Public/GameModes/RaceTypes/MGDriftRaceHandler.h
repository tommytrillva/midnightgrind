...
/**
 * @brief Quality grade for a drift based on angle and speed
 *
 * Higher grades earn points faster. The grade updates in real-time
 * during a drift as the angle changes.
 */
UENUM(BlueprintType)
enum class EMGDriftGrade : uint8
{
	/** Below minimum threshold - not scoring */
	None		UMETA(DisplayName = "None"),
	/** Basic drift: 15-30 degree slip angle */
	D			UMETA(DisplayName = "D"),
	/** Moderate drift: 30-45 degree slip angle */
	C			UMETA(DisplayName = "C"),
	/** Good drift: 45-60 degree slip angle */
	B			UMETA(DisplayName = "B"),
	/** Great drift: 60-75 degree slip angle */
	A			UMETA(DisplayName = "A"),
	/** Maximum drift: 75+ degree slip angle */
	S			UMETA(DisplayName = "S"),
	/** Perfect drift: High angle + high speed combined */
...
/**
 * @brief Real-time tracking data for an active drift
 *
 * This struct is updated every frame while a racer is drifting.
 * It contains all the information needed to calculate points and
 * display drift status to the player.
 */
...
struct FMGActiveDrift
{
	...
	/// Current phase of the drift lifecycle
	UPROPERTY(BlueprintReadOnly)
	EMGDriftState State = EMGDriftState::None;

	/// Current quality grade (updates in real-time)
	UPROPERTY(BlueprintReadOnly)
	EMGDriftGrade Grade = EMGDriftGrade::None;

	/// Current slip angle in degrees (0-90)
	UPROPERTY(BlueprintReadOnly)
	float DriftAngle = 0.0f;

	/// Current vehicle speed during drift (cm/s)
	UPROPERTY(BlueprintReadOnly)
	float Speed = 0.0f;

	/// How long this drift has been sustained (seconds)
	UPROPERTY(BlueprintReadOnly)
	float Duration = 0.0f;

	/// Points accumulated in THIS drift (not yet added to total)
	UPROPERTY(BlueprintReadOnly)
	float Points = 0.0f;

	/// Current chain multiplier (increases with consecutive drifts)
	UPROPERTY(BlueprintReadOnly)
	float Multiplier = 1.0f;

	/// Time since the last drift ended (for chain window tracking)
	UPROPERTY()
	float TimeSinceLastDrift = 0.0f;

	/// Number of drifts chained together (resets if chain breaks)
	UPROPERTY(BlueprintReadOnly)
	int32 ChainCount = 0;

	/// True if another car is drifting nearby (tandem bonus active)
	UPROPERTY(BlueprintReadOnly)
	bool bIsTandem = false;

	/** Reset drift state when drift ends (preserves chain data) */
	void Reset()
	{
		...
		Duration = 0.0f;
		Points = 0.0f;
		// Note: Multiplier and ChainCount persist to allow chaining
	}
};
