...
/**
 * EMGChallengeDifficulty - Difficulty levels for challenges
 */
UENUM(BlueprintType)
...
/**
 * FMGDailyReward - A single daily login reward
 */
...
struct FMGDailyReward
{
    ...
    UPROPERTY(EditAnywhere, BlueprintReadWrite)
    bool bIsWeeklyReward;

    UPROPERTY(EditAnywhere, BlueprintReadWrite)
    bool bIsMonthlyReward;
    ...
    UPROPERTY(EditAnywhere, BlueprintReadWrite)
    EMGDailyRewardTier RequiredTier;

    FMGDailyReward()
        : RewardId(NAME_None)
        , RewardType(EMGDailyRewardType::Currency)
        , Quantity(100)
        , DayNumber(1)
        , bIsBonusReward(false)
        , bIsWeeklyReward(false)
        , bIsMonthlyReward(false)
        , UnlockId(NAME_None)
        , RequiredTier(EMGDailyRewardTier::Bronze)
    {}
};
...
/**
 * FMGCalendarDay - A single day in the login calendar
 */
USTRUCT(BlueprintType)
...
public:
    ...
    // Tick functionality
    void TickDailyLoginSystem(float DeltaTime);
