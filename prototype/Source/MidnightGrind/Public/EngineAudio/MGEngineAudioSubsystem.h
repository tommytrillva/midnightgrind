 ...
 *
 * Determines the base character of the engine sound. Different engine types
 * have distinct sound signatures (V8 rumble vs inline-4 buzz vs rotary whine).
 *
 * @note Turbocharged and Supercharged entries represent forced induction variants
 *       that add additional audio layers on top of the base engine type.
 */
UENUM(BlueprintType)
enum class EMGEngineType : uint8
{
	Inline4,        ///< 4-cylinder inline engine (common in compact cars, high-revving)
	Inline6,        ///< 6-cylinder inline engine (smooth, BMW-style)
	V6,             ///< V6 engine (balanced, common in sports cars)
	V8,             ///< V8 engine (classic muscle car rumble)
	V10,            ///< V10 engine (exotic, high-pitched)
	V12,            ///< V12 engine (smooth, luxury sports cars)
	Flat4,          ///< Flat-4 boxer engine (Subaru-style burble)
	Flat6,          ///< Flat-6 boxer engine (Porsche-style)
	Rotary,         ///< Rotary/Wankel engine (unique high-pitched whine)
	Electric,       ///< Electric motor (whine, no combustion sounds)
	Hybrid,         ///< Hybrid powertrain (blends electric and combustion)
	Turbocharged,   ///< Adds turbo spool and blowoff sounds
	Supercharged,   ///< Adds supercharger whine
	TwinTurbo       ///< Dual turbo setup (more aggressive spool)
};
