...
struct FMGAITacticalData
{
	...
	UPROPERTY(BlueprintReadOnly, Category = "Tactical")
	EMGCatchUpMode CatchUpMode = EMGCatchUpMode::None;
...
protected:
	...
	virtual void CalculateSteering(float DeltaTime);
	...
	virtual void UpdateTactics(float DeltaTime);

	/** Update mood and learning systems (adaptive AI behavior) */
	virtual void UpdateMoodAndLearning(float DeltaTime);
	...
	virtual void HandleWaitingState(float DeltaTime);

	/** Handle normal racing state */
	virtual void HandleRacingState(float DeltaTime);

	/** Handle overtaking state */
	virtual void HandleOvertakingState(float DeltaTime);

	/** Handle position defense state */
	virtual void HandleDefendingState(float DeltaTime);

	/** Handle collision/off-track recovery */
	virtual void HandleRecoveringState(float DeltaTime);

	/** Handle pushing hard (skill-based catch-up) */
	virtual void HandlePushingHardState(float DeltaTime);

	/** Handle managing lead (conservative driving when ahead) */
	virtual void HandleManagingLeadState(float DeltaTime);

	/** Handle drafting state */
	virtual void HandleDraftingState(float DeltaTime);
	...
	virtual EMGCatchUpMode DetermineCatchUpMode() const;
