...
void AMGPlayerState::CopyProperties(APlayerState* PlayerState)
{
	...
	if (AMGPlayerState* MGPlayerState = Cast<AMGPlayerState>(PlayerState))
	{
		...
		MGPlayerState->PlatformID = PlatformID;
		MGPlayerState->ProfileLevel = ProfileLevel;
		MGPlayerState->CrewName = CrewName;
		MGPlayerState->VehicleSelection = VehicleSelection;
	}
}
