...
FMGVehicleStats UMGStatsTracker::GetVehicleStats(FName VehicleID) const
{
	if (const FMGVehicleStats* Stats = VehicleStatsMap.Find(VehicleID))
	{
		return *Stats;
	}
	FMGVehicleStats Empty;
	...
	float RaceTime, float BestLap, float DriftScore, bool bPinkSlip)
{
	...
	UpdateVehicleStats(VehicleID, [&](FMGVehicleStats& Stats)
	{
		Stats.RacesEntered++;
		if (bWon) Stats.Wins++;
...
void UMGStatsTracker::RecordDrift(float Score, float Duration, int32 ComboCount)
{
	...
	{
		UpdateVehicleStats(CurrentVehicle, [&](FMGVehicleStats& Stats)
		{
...
void UMGStatsTracker::UpdateVehicleStats(FName VehicleID, const TFunction<void(FMGVehicleStats&)>& UpdateFunc)
{
	if (VehicleID.IsNone())
	{
		return;
	...
	FMGVehicleStats& Stats = VehicleStatsMap.FindOrAdd(VehicleID);
