...
// EMGPursuitState - REMOVED (duplicate)
...
// FMGPursuitStatus - MOVED TO HeatLevel/MGHeatLevelSubsystem.h
// Canonical definition in: HeatLevel/MGHeatLevelSubsystem.h
...
/**
 * @brief Wrapper for TArray<FMGRoadblock> to support UPROPERTY in TMap.
 */
USTRUCT(BlueprintType)
struct FMGRoadblockArray
{
	GENERATED_BODY()

	UPROPERTY(EditAnywhere, BlueprintReadWrite)
	TArray<FMGRoadblock> Roadblocks;
};

/**
 * @brief Wrapper for TArray<FMGPursuitEvent> to support UPROPERTY in TMap.
 */
USTRUCT(BlueprintType)
struct FMGPursuitEventArray
{
	GENERATED_BODY()

	UPROPERTY(EditAnywhere, BlueprintReadWrite)
	TArray<FMGPursuitEvent> Events;
};
...
public:
	...
	void UpdatePursuit(const FString& PlayerId, FVector PlayerLocation, FVector PlayerVelocity, float MGDeltaTime);
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Pursuit|Update")
	void UpdateUnitAI(const FString& PlayerId, float MGDeltaTime);
...
protected:
	...
	void UpdateEscapeProgress(const FString& PlayerId, FVector PlayerLocation, float MGDeltaTime);

	/// Updates bust meter based on player proximity to units
	void UpdateBustedProgress(const FString& PlayerId, float MGDeltaTime);
...
private:
	...
	UPROPERTY()
	TMap<FString, FMGRoadblockArray> ActiveRoadblocks;

	/// Map of player ID to their pursuit event history
	UPROPERTY()
	TMap<FString, FMGPursuitEventArray> PursuitEvents;
