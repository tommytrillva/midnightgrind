...
#include "Takedown/MGTakedownSubsystem.h"  // FMGTakedownEvent
...
struct FMGCollisionStats
{
	...
	/** Highest takedown chain achieved (consecutive takedowns) */
	UPROPERTY(EditAnywhere, BlueprintReadWrite)
	int32 BestTakedownChain = 0;

	/** Total number of revenge takedowns completed */
	UPROPERTY(EditAnywhere, BlueprintReadWrite)
	int32 TotalRevenges = 0;
};

// NOTE: FMGTakedownEvent is now defined in Takedown/MGTakedownSubsystem.h
...
/**
 * @brief Wrapper for TArray<FMGCollisionEvent> to support UPROPERTY in TMap.
 */
USTRUCT(BlueprintType)
struct FMGCollisionEventArray
{
	GENERATED_BODY()

	UPROPERTY(EditAnywhere, BlueprintReadWrite)
	TArray<FMGCollisionEvent> Events;
};
...
public:
	...
	void UpdateCollisionSystem(float MGDeltaTime);
...
protected:
	...
	void TickCollision(float MGDeltaTime);

	/** Update invincibility timers and expire them when done */
	void UpdateInvincibility(float MGDeltaTime);

	/** Update takedown chain timers and reset expired chains */
	void UpdateTakedownChains(float MGDeltaTime);

	/** Apply ongoing fire damage to burning vehicles */
	void UpdateFireDamage(float MGDeltaTime);
...
private:
	...
	UPROPERTY()
	TMap<FString, FMGCollisionEventArray> RecentCollisions;
