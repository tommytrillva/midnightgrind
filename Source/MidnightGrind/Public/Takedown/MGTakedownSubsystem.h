...
/**
 * Lifetime takedown statistics (persisted across sessions)
 */
USTRUCT(BlueprintType)
struct FMGTakedownLifetimeStats
{
	GENERATED_BODY()
...
protected:
	void TickAggression(float MGDeltaTime);
	void TickStreak(float MGDeltaTime);
...
private:
	...
	/** Lifetime statistics persisted across sessions */
	UPROPERTY()
	FMGTakedownLifetimeStats LifetimeStats;
