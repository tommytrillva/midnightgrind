...
// EMGReputationTier - defined in Core/MGSharedTypes.h
// Tiers: Unknown, Rookie, Known, Respected, Feared, Legend
// Per PRD Section 4.2 - tier thresholds are defined in TierThresholds array
 ...
 *
 * @note This struct is shared between MGContentGatingSubsystem and MGPlayerProgression.
 */
...
struct FMGUnlockRequirement
{
	...
	// ---- Tier & REP Requirements ----

	/// Minimum reputation tier the player must reach
	...
	/// Exact amount of REP points required (cumulative, not per-tier)
	/// This is separate from tier - you might require a specific REP amount
	/// Example: 2500 REP even though Respected only needs 1500
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Tier")
	int32 RequiredREP = 0;

	// ---- Level Requirements (used by MGPlayerProgression) ----

	/// Minimum player level required to unlock this content
	/// Example: Level 10 to access advanced tuning
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Level")
	int32 RequiredLevel = 0;

	// ---- Crew Requirements (used by MGPlayerProgression) ----

	/// Crew ID the player must have reputation with (0 = None)
	/// Maps to EMGCrew enum: 1=Midnight, 2=Velocity, 3=Chrome, 4=Shadow, 5=Apex
	/// If 0, no specific crew is required
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Crew")
	uint8 RequiredCrewID = 0;

	/// Minimum reputation points with the required crew
	/// Only checked if RequiredCrewID != 0
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Crew")
	...
	/// Alias for RequiredRaceWins for MGPlayerProgression compatibility
	/// @deprecated Use RequiredRaceWins instead
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Races", meta = (DisplayName = "Required Wins"))
	int32 RequiredWins = 0;

	/// Total number of races completed required
	/// Example: 50 races to unlock competitive mode
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Races")
	int32 RequiredRaces = 0;

	// ---- Prerequisite Unlocks ----

	/// ID of another unlock that must be acquired first (single)
	...
	/// Array of unlock IDs that must all be acquired first (multiple)
	/// Used when multiple prerequisites are needed
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Prerequisites")
	...
	// ---- For Registration (used by MGPlayerProgression) ----

	/// Unique ID for this unlock requirement registration
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Registration")
	FName UnlockID;

	/// Type of content being unlocked (maps to EMGUnlockType in MGPlayerProgression.h)
	/// 0=Vehicle, 1=Part, 2=Track, 3=RaceType, 4=Crew, 5=Cosmetic, 6=Feature
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Registration")
	uint8 UnlockTypeID = 0;

	/// Display name for the unlock
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Registration")
	FText DisplayName;
};
