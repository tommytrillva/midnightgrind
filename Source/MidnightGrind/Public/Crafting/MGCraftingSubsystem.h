...
/**
 * @brief Wrapper for TArray<FMGUpgradeSlot> to support UPROPERTY in TMap.
 */
USTRUCT(BlueprintType)
struct FMGUpgradeSlotArray
{
	GENERATED_BODY()

	UPROPERTY(EditAnywhere, BlueprintReadWrite)
	TArray<FMGUpgradeSlot> Slots;
};
...
public:
    ...
    void TickCrafting(float MGDeltaTime);
...
private:
    ...
    UPROPERTY()
    TMap<FName, FMGUpgradeSlotArray> ItemUpgrades;
