...
/**
 * @brief Types of violations that can result in penalties
 */
UENUM(BlueprintType)
enum class EMGPenaltyViolationType : uint8
{
    None,            ///< No specific violation
    TrackLimits,     ///< Exceeding track boundaries
    Collision,       ///< At-fault collision with another driver
    FalseStart,      ///< Moving before start signal
    PitSpeeding,     ///< Exceeding pit lane speed limit
    IgnoringFlags,   ///< Ignoring blue flags or other race control flags
    UnservedPenalty, ///< Failing to serve a penalty in time
...
struct FMGPenalty
{
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite)
	EMGPenaltyViolationType Violation = EMGPenaltyViolationType::None;
...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_TwoParams(FOnWarningIssued, FName, VehicleID, EMGPenaltyViolationType, Violation);
...
public:
	...
	FMGPenalty IssuePenalty(FName VehicleID, EMGPenaltyType Type, EMGPenaltyViolationType Violation, float TimeValue = 0.0f);

	/** Issues an official warning without time/position penalty */
	UFUNCTION(BlueprintCallable, Category = "Penalty|Issue")
	void IssueWarning(FName VehicleID, EMGPenaltyViolationType Violation);

	/** Issues a drive-through penalty (pass through pit lane) */
	UFUNCTION(BlueprintCallable, Category = "Penalty|Issue")
	void IssueDriveThrough(FName VehicleID, EMGPenaltyViolationType Violation);

	/** Issues a stop-and-go penalty with specified stop duration */
	UFUNCTION(BlueprintCallable, Category = "Penalty|Issue")
	void IssueStopAndGo(FName VehicleID, EMGPenaltyViolationType Violation, float Duration = 10.0f);

	/** Issues a time penalty to be added to race result */
	UFUNCTION(BlueprintCallable, Category = "Penalty|Issue")
	void IssueTimePenalty(FName VehicleID, EMGPenaltyViolationType Violation, float Seconds);

	/** Issues a position drop penalty during the race */
	UFUNCTION(BlueprintCallable, Category = "Penalty|Issue")
	void IssuePositionPenalty(FName VehicleID, EMGPenaltyViolationType Violation, int32 Positions);

	/** Disqualifies driver from current race */
	UFUNCTION(BlueprintCallable, Category = "Penalty|Issue")
	void IssueDisqualification(FName VehicleID, EMGPenaltyViolationType Violation);
