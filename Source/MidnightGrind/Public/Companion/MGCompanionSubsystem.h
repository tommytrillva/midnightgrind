...
/**
 * @brief Wrapper for TMap<FName, float> to support UPROPERTY in TMap.
 */
USTRUCT(BlueprintType)
struct FMGAbilityCooldownMap
{
	GENERATED_BODY()
...
/**
 * @brief Wrapper for TArray<FMGCompanionDialogue> to support UPROPERTY in TMap.
 */
USTRUCT(BlueprintType)
struct FMGCompanionDialogueArray
{
	GENERATED_BODY()

	UPROPERTY(EditAnywhere, BlueprintReadWrite)
	TArray<FMGCompanionDialogue> Dialogues;
};
...
public:
    ...
    void TickCompanions(float MGDeltaTime);
...
private:
    ...
    UPROPERTY()
    TMap<FName, FMGAbilityCooldownMap> AbilityCooldowns;

    UPROPERTY()
    TMap<FName, FMGCompanionDialogueArray> CompanionDialogues;
