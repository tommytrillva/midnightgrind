...
/**
 * @brief Wrapper for TMap<EMGPrestigeCategory, FMGCategoryPrestige> to support UPROPERTY in TMap.
 */
USTRUCT(BlueprintType)
struct FMGCategoryPrestigeMap
{
	GENERATED_BODY()

	UPROPERTY(EditAnywhere, BlueprintReadWrite)
	TMap<EMGPrestigeCategory, FMGCategoryPrestige> Categories;
};
...
/**
 * @brief Wrapper for TMap<FString, int32> to support UPROPERTY in TMap.
 */
USTRUCT(BlueprintType)
struct FMGPurchaseCountMap
{
	GENERATED_BODY()
...
private:
	...
	UPROPERTY()
	TMap<FString, FMGCategoryPrestigeMap> CategoryPrestigeData;

	UPROPERTY()
	TMap<FString, FMGPrestigePlayerStats> PlayerStats;

	UPROPERTY()
	TMap<FString, FMGPrestigeStringSet> AchievedMilestones;

	UPROPERTY()
	TMap<FString, FMGPurchaseCountMap> ShopPurchaseCounts;
