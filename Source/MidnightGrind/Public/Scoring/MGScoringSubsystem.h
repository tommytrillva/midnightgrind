...
/**
 * @brief Wrapper for TArray<FMGScoreMultiplierSource> to support UPROPERTY in TMap.
 */
USTRUCT(BlueprintType)
struct FMGScoreMultiplierSourceArray
{
	GENERATED_BODY()

	UPROPERTY(EditAnywhere, BlueprintReadWrite)
	TArray<FMGScoreMultiplierSource> Sources;
};

/**
 * @brief Wrapper for TSet<int32> to support UPROPERTY in TMap.
 */
USTRUCT(BlueprintType)
struct FMGInt32Set
{
...
public:
	...
	void UpdateScoringSystem(float MGDeltaTime);

protected:
	void TickChains(float MGDeltaTime);
	void TickMultipliers(float MGDeltaTime);
...
private:
	...
	UPROPERTY()
	TMap<FString, FMGScoreMultiplierSourceArray> PlayerMultipliers;

	UPROPERTY()
	TMap<FString, FString> ActiveRaceIds;

	UPROPERTY()
	TMap<FString, FMGInt32Set> ReachedMilestones;
