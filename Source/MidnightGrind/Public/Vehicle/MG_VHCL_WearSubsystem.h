...
#include "MG_VHCL_WearSubsystem.generated.h"
...
public:
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Wear")
	void UpdateWearFromGameplay(FGuid VehicleID, float MGDeltaTime, const FVector& Velocity,
	...
	void ApplyTireWear(FGuid VehicleID, float WheelSlip, float Speed, bool bDrifting, float MGDeltaTime);
	...
	void ApplyEngineWear(FGuid VehicleID, float RPM, float MaxRPM, float Throttle, float MGDeltaTime);
	...
	void ApplyBrakeWear(FGuid VehicleID, float BrakeForce, float Speed, float MGDeltaTime);
...
protected:
	...
	void UpdateEngineState(FMGEngineWearData& Engine, float MGDeltaTime);
	void UpdateBrakeState(FMGBrakeWearData& Brakes, float MGDeltaTime);
