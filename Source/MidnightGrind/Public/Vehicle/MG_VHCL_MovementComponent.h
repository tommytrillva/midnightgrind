...
#pragma once
/**
 * @file MG_VHCL_MovementComponent.h
...
#include "MG_VHCL_Data.h"
#include "MG_TIRE_PressureTypes.h"
#include "MGPhysicsConstants.h"
#include "RacingWheel/MGRacingWheelTypes.h"
#include "MG_VHCL_MovementComponent.generated.h"
...
/**
 * @brief Current drift state tracking
 *
 * Tracks all aspects of a drift including angle, duration, score, and chain.
 * Used by the scoring system and HUD.
 */
...
struct FMGDriftState
{
	...
	/** Duration of current drift in seconds */
	UPROPERTY(BlueprintReadOnly, Category = "Drift")
	...
	/** Current chain/combo multiplier */
	UPROPERTY(BlueprintReadOnly, Category = "Drift")
	int32 ChainMultiplier = 1;

	/** Total chain score (multiple drifts linked) */
	UPROPERTY(BlueprintReadOnly, Category = "Drift")
	float ChainScore = 0.0f;

	/** Speed at drift initiation */
	UPROPERTY(BlueprintReadOnly, Category = "Drift")
...
/**
 * @brief Tire temperature state for a single tire
 *
 * Tracks inner, middle, and outer tread temperatures.
 * Temperature affects grip and wear rate.
 */
USTRUCT(BlueprintType)
struct FMGTireTemperature
{
	...
	/** Inner tread temperature in Celsius */
	UPROPERTY(BlueprintReadOnly, Category = "Tire Temperature")
	float InnerTemp = 25.0f;

	/** Middle tread temperature in Celsius */
	UPROPERTY(BlueprintReadOnly, Category = "Tire Temperature")
	float MiddleTemp = 25.0f;

	/** Outer tread temperature in Celsius */
	UPROPERTY(BlueprintReadOnly, Category = "Tire Temperature")
	float OuterTemp = 25.0f;

	/** Core/carcass temperature in Celsius */
	UPROPERTY(BlueprintReadOnly, Category = "Tire Temperature")
	float CoreTemp = 25.0f;

	/** Average surface temperature */
	float GetAverageTemp() const { return (InnerTemp + MiddleTemp + OuterTemp) / 3.0f; }

	/** Whether tire is in optimal temperature window */
	bool IsInOptimalRange(float OptimalMin = 80.0f, float OptimalMax = 100.0f) const
	{
		float avg = GetAverageTemp();
		return avg >= OptimalMin && avg <= OptimalMax;
	}
};

/**
 * @brief Tire pressure runtime state
 *
 * Current pressure state for a single tire including pressure, status, and leak info.
 */
USTRUCT(BlueprintType)
struct FMGTirePressureState
{
	...
	UPROPERTY(BlueprintReadOnly, Category = "Tire Pressure")
	float CurrentPressurePSI = 32.0f;

	/** Target/optimal pressure in PSI */
	UPROPERTY(BlueprintReadOnly, Category = "Tire Pressure")
	float TargetPressurePSI = 32.0f;

	/** Critical low pressure threshold */
	UPROPERTY(BlueprintReadOnly, Category = "Tire Pressure")
	float CriticalLowPressurePSI = 18.0f;

	/** Whether tire is flat/blown */
	UPROPERTY(BlueprintReadOnly, Category = "Tire Pressure")
	bool bIsFlat = false;

	/** Current leak cause if any */
	UPROPERTY(BlueprintReadOnly, Category = "Tire Pressure")
	EMGPressureLossCause LeakCause = EMGPressureLossCause::None;

	/** Leak rate in PSI per second */
	UPROPERTY(BlueprintReadOnly, Category = "Tire Pressure")
	float LeakRatePSIPerSec = 0.0f;
};

/**
 * @brief Weight transfer state
 *
 * Tracks dynamic weight distribution during acceleration, braking, and cornering.
 * Affects grip available at each wheel.
 */
USTRUCT(BlueprintType)
struct FMGWeightTransfer
{
	...
	/** Longitudinal weight transfer (positive = forward/braking) */
	UPROPERTY(BlueprintReadOnly, Category = "Weight Transfer")
	float LongitudinalTransfer = 0.0f;

	/** Lateral weight transfer (positive = right/left turn) */
	UPROPERTY(BlueprintReadOnly, Category = "Weight Transfer")
	float LateralTransfer = 0.0f;

	/** Current load on front left wheel (Newtons) */
	UPROPERTY(BlueprintReadOnly, Category = "Weight Transfer")
	float FrontLeftLoad = 0.0f;

	/** Current load on front right wheel (Newtons) */
	UPROPERTY(BlueprintReadOnly, Category = "Weight Transfer")
	float FrontRightLoad = 0.0f;

	/** Current load on rear left wheel (Newtons) */
	UPROPERTY(BlueprintReadOnly, Category = "Weight Transfer")
	float RearLeftLoad = 0.0f;

	/** Current load on rear right wheel (Newtons) */
	UPROPERTY(BlueprintReadOnly, Category = "Weight Transfer")
	float RearRightLoad = 0.0f;

	/** Current front weight percentage (0-1) */
	UPROPERTY(BlueprintReadOnly, Category = "Weight Transfer")
	float FrontWeightPercent = 0.55f;
};
...
struct FMGPowerDistributionData
{
	...
	/** Per-wheel spin status (1 if significant wheelspin, 0 otherwise) */
	UPROPERTY()
	uint8 WheelSpinning[4] = { 0, 0, 0, 0 };
};
...
public:
	 ...
	 */
	virtual void SetHandbrakeInput(bool bEngaged) override;
	...
	virtual int32 GetCurrentGear() const override { return CurrentGear; }
	...
	float MGDownforceCoefficient = 0.3f; // Base downforce factor

	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Tuning|Aero")
	float MGDragCoefficient = 0.35f; // Air resistance factor
...
protected:
	...
	virtual void UpdateEngineSimulation(float MGDeltaTime);

	/** Update turbo/boost simulation with advanced shaft modeling */
	virtual void UpdateBoostSimulation(float MGDeltaTime);

	/** Update drift detection and enhanced scoring system */
	virtual void UpdateDriftPhysics(float MGDeltaTime);

	/** Update nitrous system */
	virtual void UpdateNitrousSystem(float MGDeltaTime);

	/** Apply stability control */
	virtual void ApplyStabilityControl(float MGDeltaTime);

	/** Apply anti-flip torque */
	virtual void ApplyAntiFlipForce(float MGDeltaTime);
	 ...
	 */
	virtual void UpdateSurfaceDetection(float MGDeltaTime);

	/** Perform gear shift */
	void PerformGearShift(int32 NewGear);

	/** Update tire temperature simulation */
	virtual void UpdateTireTemperatures(float MGDeltaTime);
	 ...
	 */
	virtual void UpdateTirePressure(float MGDeltaTime);
	 ...
	 */
	bool CheckAndApplyBlowoutRisk(int32 WheelIndex, float MGDeltaTime);
	...
	virtual void UpdateWeightTransfer(float MGDeltaTime);

	/** Update clutch wear and temperature simulation */
	virtual void UpdateClutchWear(float MGDeltaTime);

	/** Update aerodynamic forces */
	virtual void UpdateAerodynamics(float MGDeltaTime);

	/** Update anti-lag system with improved turbo modeling */
	virtual void UpdateAntiLag(float MGDeltaTime);

	/** Update launch control */
	virtual void UpdateLaunchControl(float MGDeltaTime);

	/** Update brake temperature and fade */
	virtual void UpdateBrakeSystem(float MGDeltaTime);

	/** Apply differential behavior based on type */
	virtual void ApplyDifferentialBehavior(float MGDeltaTime);
	 ...
	 * @param DeltaTime Frame delta time
	 */
	void UpdateWheelAngularVelocities(float MGDeltaTime);
	 ...
	 * @param DeltaTime Frame delta time
	 */
	void UpdatePowerDistributionData(float MGDeltaTime);
	 ...
	 * @param DeltaTime Frame delta time
	 */
	void IntegrateDifferentialWithWeightTransfer(float MGDeltaTime);
	 ...
	 * @param DeltaTime Frame delta time
	 */
	void UpdateDriftScoring(float MGDeltaTime);
	 ...
	 * @param DeltaTime Frame delta time
	 */
	void UpdateTurboShaftSimulation(float MGDeltaTime);
	 ...
	 * @param DeltaTime Frame delta time
	 */
	virtual void UpdateSuspensionGeometry(float MGDeltaTime);
	 ...
	 * @param DeltaTime Frame delta time
	 */
	void ApplySteeringSelfCentering(float MGDeltaTime);
