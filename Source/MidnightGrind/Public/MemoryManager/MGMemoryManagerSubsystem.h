 ...
 * @see FMGStreamingSettings For global streaming configuration
 */
...
/**
 * Streaming settings for memory management
 * NOTE: Renamed from FStreamingSettings to avoid conflict with engine's UStreamingSettings
 */
USTRUCT(BlueprintType)
struct FMGStreamingSettings
{
    GENERATED_BODY()
...
public:
    ...
    FMGStreamingSettings GetStreamingSettings() const { return StreamingSettings; }

    UFUNCTION(BlueprintCallable, Category = "Memory|Settings")
    void UpdateStreamingSettings(const FMGStreamingSettings& NewSettings);
...
private:
    ...
    UPROPERTY()
    FMGStreamingSettings StreamingSettings;
