...
// FMGSlipstreamConfig - defined in Slipstream/MGSlipstreamSubsystem.h
...
public:
	...
	void UpdateVehicleAero(const FString& VehicleId, FVector Position, FVector Velocity, float MGDeltaTime);
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Aerodynamics|Update")
	void UpdateAerodynamics(float MGDeltaTime);
...
protected:
	...
	void TickAerodynamics(float MGDeltaTime);

	/// Updates all active slipstream sessions.
	void UpdateSlipstreams(float MGDeltaTime);

	/// Applies wind zone effects to vehicles.
	void UpdateWindEffects(float MGDeltaTime);

	/// Calculates all aero forces for a single vehicle.
	void CalculateAeroForces(const FString& VehicleId, float MGDeltaTime);
	...
	void UpdateSlipstreamState(FMGSlipstreamSession& Session, float Distance, float MGDeltaTime);
