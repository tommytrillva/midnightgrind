...
#include "LiveService/MGLiveEventsSubsystem.h"  // FMGPlaylistEntry (canonical)
#include "Drift/MGDriftSubsystem.h"  // FMGDriftLeaderboardEntry
...
// EMGCatchUpMode - REMOVED (duplicate)
// Canonical definition in: AI/MGAIRacerController.h
...
// FMGDriftLeaderboardEntry is defined in Drift/MGDriftSubsystem.h
...
public:
	...
	void UpdateDriftScore(const FString& PlayerID, float DriftAngle, float Speed, float MGDeltaTime);

	UFUNCTION(BlueprintCallable, Category = "GameMode|Drift")
	void EndDriftCombo(const FString& PlayerID);

	UFUNCTION(BlueprintPure, Category = "GameMode|Drift")
	TArray<FMGDriftLeaderboardEntry> GetDriftLeaderboard() const;
