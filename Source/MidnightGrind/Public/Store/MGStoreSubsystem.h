...
// EMGItemRarity - MOVED TO MGSharedTypes.h
...
public:
	...
	/** Returns all items in the Featured category for homepage display */
	UFUNCTION(BlueprintPure, Category = "Store")
	TArray<FMGStoreItem> GetFeaturedItems() const;

	/** Returns all items matching the specified store category */
	UFUNCTION(BlueprintPure, Category = "Store")
	TArray<FMGStoreItem> GetItemsByCategory(EMGStoreCategory Category) const;

	/** Looks up a specific item by its unique ItemID. Returns empty item if not found. */
	UFUNCTION(BlueprintPure, Category = "Store")
	FMGStoreItem GetItem(FName ItemID) const;

	/** Returns the current daily deals with their discounted prices */
	UFUNCTION(BlueprintPure, Category = "Store")
	TArray<FMGDailyDeal> GetDailyDeals() const { return CurrentDailyDeals; }

	/** Returns time remaining until daily deals refresh (midnight UTC) */
	UFUNCTION(BlueprintPure, Category = "Store")
	FTimespan GetTimeUntilDailyRefresh() const;

	// Purchasing

	/** Attempts to purchase an item using GrindCash (earnable currency) */
	UFUNCTION(BlueprintCallable, Category = "Store")
	FMGPurchaseResult PurchaseWithGrindCash(FName ItemID);

	/** Attempts to purchase an item using NeonCredits (premium currency) */
	UFUNCTION(BlueprintCallable, Category = "Store")
	FMGPurchaseResult PurchaseWithNeonCredits(FName ItemID);

	/** Purchases a daily deal at its discounted price. DealIndex is 0-based index into GetDailyDeals() */
	UFUNCTION(BlueprintCallable, Category = "Store")
	FMGPurchaseResult PurchaseDailyDeal(int32 DealIndex, bool bUseNeonCredits = false);

	/** Checks if the player has enough currency to purchase the specified item */
	UFUNCTION(BlueprintPure, Category = "Store")
	bool CanAffordItem(FName ItemID, bool bUseNeonCredits = false) const;

	// Ownership

	/** Returns true if the player already owns the specified item */
	UFUNCTION(BlueprintPure, Category = "Store")
	bool OwnsItem(FName ItemID) const;

	/** Returns all items the player owns, with acquisition metadata */
	UFUNCTION(BlueprintPure, Category = "Store")
	TArray<FMGOwnedItem> GetOwnedItems() const { return OwnedItems; }

	/** Returns owned items filtered by store category */
	UFUNCTION(BlueprintPure, Category = "Store")
	TArray<FMGOwnedItem> GetOwnedItemsByCategory(EMGStoreCategory Category) const;

	/** Directly unlocks an item without purchase. Used for gifts, achievements, events. */
	UFUNCTION(BlueprintCallable, Category = "Store")
	void UnlockItem(FName ItemID, const FString& Method, const FString& Source = TEXT(""));

	// Gifting

	/** Purchases an item as a gift for another player. Deducts from sender's currency. */
	...
	/** Returns all gifts waiting to be claimed by the current player */
	UFUNCTION(BlueprintPure, Category = "Store|Gifting")
	TArray<FMGGiftTransaction> GetPendingGifts() const { return PendingGifts; }

	/** Claims a pending gift, unlocking the item for the player */
	UFUNCTION(BlueprintCallable, Category = "Store|Gifting")
	bool ClaimGift(const FString& GiftID);

	/** Returns true if the specified item is eligible for gifting */
	UFUNCTION(BlueprintPure, Category = "Store|Gifting")
	bool CanGiftItem(FName ItemID) const;

	// Bundle Value

	/** Calculates total GrindCash value if all bundle items were purchased individually */
	UFUNCTION(BlueprintPure, Category = "Store|Bundle")
	int64 GetBundleValue(FName BundleID) const;

	/** Returns the percentage saved by buying the bundle vs individual items */
	UFUNCTION(BlueprintPure, Category = "Store|Bundle")
	float GetBundleSavingsPercent(FName BundleID) const;

	/** Returns bundle items the player doesn't already own */
	UFUNCTION(BlueprintPure, Category = "Store|Bundle")
