 ...
 *
 * VEHICLE-SPECIFIC STATS (FMGVehicleSpecs):
 ...
 * FMGVehicleSpecs VehicleStats = Stats->GetVehicleStats(VehicleID);
 * @endcode
 *
 * BEST PRACTICES:
 * ---------------
...
/**
 * Per-vehicle racing statistics - tracks performance history for each vehicle.
 *
 * Helps players see which cars they perform best with. Stored per VehicleID
 * and updated after each race.
 *
 * NOTE: This is distinct from FMGVehicleSpecs (defined in Vehicle/MG_VHCL_Data.h)
 * which holds calculated performance metrics (horsepower, handling rating, etc.)
 *
 * @see FMGVehicleSpecs for vehicle performance specifications
 */
USTRUCT(BlueprintType)
struct FMGVehicleRacingStats
{
	GENERATED_BODY()
	...
	/** Total races entered with this vehicle */
	UPROPERTY(BlueprintReadOnly, Category = "Races")
	int32 RacesEntered = 0;

	/** Number of wins with this vehicle */
	UPROPERTY(BlueprintReadOnly, Category = "Races")
	int32 Wins = 0;

	/** Number of podium finishes (top 3) */
	UPROPERTY(BlueprintReadOnly, Category = "Races")
	int32 Podiums = 0;

	/** Total distance driven with this vehicle (cm) */
	UPROPERTY(BlueprintReadOnly, Category = "Driving")
	float DistanceDriven = 0.0f;

	/** Best lap time achieved with this vehicle (seconds) */
	UPROPERTY(BlueprintReadOnly, Category = "Times")
	float BestLapTime = 0.0f;

	/** Track where best lap was set */
	UPROPERTY(BlueprintReadOnly, Category = "Times")
	FName BestLapTrack;
	...
	/** Highest top speed achieved (cm/s) */
	UPROPERTY(BlueprintReadOnly, Category = "Performance")
	float TopSpeed = 0.0f;

	/** Total drift score accumulated */
	UPROPERTY(BlueprintReadOnly, Category = "Drifting")
	float TotalDriftScore = 0.0f;

	/** Highest single drift score */
	UPROPERTY(BlueprintReadOnly, Category = "Drifting")
	float BestDriftScore = 0.0f;

	/** Total damage taken with this vehicle */
	UPROPERTY(BlueprintReadOnly, Category = "Damage")
	int32 TotalDamage = 0;

	/** Calculate win rate for this vehicle */
	float GetWinRate() const { return RacesEntered > 0 ? static_cast<float>(Wins) / RacesEntered : 0.0f; }

	/** Get distance in miles */
	float GetDistanceInMiles() const { return DistanceDriven / 160934.0f; }
};
...
public:
	...
	/** Get racing statistics for a specific vehicle */
	UFUNCTION(BlueprintPure, Category = "Stats")
	FMGVehicleRacingStats GetVehicleStats(FName VehicleID) const;
...
protected:
	...
	void UpdateVehicleStats(FName VehicleID, const TFunction<void(FMGVehicleRacingStats&)>& UpdateFunc);
...
private:
	...
	TMap<FName, FMGVehicleRacingStats> VehicleStatsMap;
