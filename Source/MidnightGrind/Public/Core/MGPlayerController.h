...
#include "Core/MGSharedTypes.h" // Master shared types (EMGHeatLevel, EMGCurrencyType, EMGTimeOfDay, etc.)
#include "Input/MGVehicleInputHandler.h" // Canonical definition of FMGVehicleInputState
#include "Airtime/MGAirtimeSubsystem.h"
#include "Bonus/MGBonusSubsystem.h" // Canonical definition of FMGBonusDefinition
#include "Bounty/MGBountySubsystem.h" // Canonical definition of FMGBountyCompletionResult
#include "Career/MGCareerSubsystem.h" // EMGCareerChapter, EMGCareerMilestone, FMGCareerObjective
#include "Caution/MGCautionSubsystem.h" // EMGCautionType, EMGCautionReason, FMGSafetyCarState
#include "Challenges/MGChallengeSubsystem.h" // FMGChallenge (canonical)
#include "Checkpoint/MGCheckpointSubsystem.h" // FMGCheckpointPassage, FMGLapData (canonical)
#include "Contract/MGContractSubsystem.h" // FMGContract, FMGContractObjective (canonical)
#include "Drift/MGDriftSubsystem.h" // FMGDriftResult, EMGDriftGrade (canonical)
#include "Fuel/MG_FUEL_Subsystem.h" // EMGFuelAlert, EMGFuelState, EMGFuelMode (canonical)
#include "Tire/MGTireSubsystem.h" // EMGTirePosition, EMGTireCondition (canonical)
#include "Takedown/MGTakedownSubsystem.h" // EMGTakedownType, FMGTakedownEvent (canonical)
#include "PitStop/MGPitStopSubsystem.h" // EMGPitLaneViolation, FMGPitStopResult (canonical)
#include "Speedtrap/MGSpeedtrapSubsystem.h" // EMGSpeedtrapRating (canonical)
#include "Destruction/MGDestructionSubsystem.h" // FMGDestructionEvent (canonical)
#include "Scoring/MGScoringSubsystem.h" // FMGScoreEvent (canonical)
#include "NearMiss/MGNearMissSubsystem.h" // FMGNearMissEvent (canonical)
#include "Pursuit/MGPursuitSubsystem.h" // EMGPursuitIntensity, FMGPursuitUnit (canonical)
#include "Stunt/MGStuntSubsystem.h" // FMGStuntEvent (canonical)
#include "Powerup/MGPowerupSubsystem.h" // EMGPowerupType (canonical)
#include "Penalty/MGPenaltySubsystem.h" // EMGPenaltyType, FMGPenalty (canonical)
#include "Streak/MGStreakSubsystem.h" // EMGStreakType, EMGStreakTier (canonical)
#include "Prestige/MGPrestigeSubsystem.h" // EMGPrestigeRank (canonical)
#include "RaceDirector/MGRaceDirectorSubsystem.h" // FMGRaceEvent, EMGDramaticMoment (canonical)
#include "Race/MGRaceStarter.h" // EMGRaceEventType (canonical)
#include "License/MGLicenseSubsystem.h" // EMGLicenseCategory, EMGLicenseTier, EMGTestGrade (canonical)
#include "Currency/MGCurrencySubsystem.h" // FMGEarningMultiplier (canonical)
#include "Ghost/MGGhostSubsystem.h" // FMGGhostComparator, EMGGhostComparison (canonical)
#include "Rivals/MGRivalsSubsystem.h" // FMGRival (canonical)
#include "DailyRewards/MGDailyRewardsSubsystem.h" // FMGRewardClaimResult, EMGStreakMilestone, FMGDailyReward (canonical)
#include "Slipstream/MGSlipstreamSubsystem.h" // EMGDraftingZone, EMGSlipstreamStrength (canonical)
#include "Reputation/MGReputationSubsystem.h" // EMGReputationCategory, FMGReputationUnlock (canonical)
...
// FMGNearMissEvent - defined in NearMiss/MGNearMissSubsystem.h (now included)
// FMGDriftResult - defined in Drift/MGDriftSubsystem.h
// FMGJumpResult - defined in Airtime/MGAirtimeSubsystem.h
// EMGTrickType - defined in Core/MGSharedTypes.h (included above)
// EMGFuelAlert - defined in Fuel/MG_FUEL_Subsystem.h
// EMGTirePosition - defined in Tire/MGTireSubsystem.h
// EMGTireCondition - defined in Tire/MGTireSubsystem.h
// EMGTakedownType - defined in Takedown/MGTakedownSubsystem.h
// EMGPitLaneViolation - defined in PitStop/MGPitStopSubsystem.h
// EMGPursuitIntensity - defined in Pursuit/MGPursuitSubsystem.h
// FMGTakedownEvent - defined in Takedown/MGTakedownSubsystem.h
// FMGPitStopResult - defined in PitStop/MGPitStopSubsystem.h
// FMGBonusDefinition - defined in Bonus/MGBonusSubsystem.h
// FMGPursuitUnit - defined in Pursuit/MGPursuitSubsystem.h
// EMGSpeedtrapRating - defined in Speedtrap/MGSpeedtrapSubsystem.h
// FMGDestructionEvent - defined in Destruction/MGDestructionSubsystem.h
// FMGScoreEvent - defined in Scoring/MGScoringSubsystem.h
struct FMGAchievementDefinition;
// EMGStreakType - defined in Streak/MGStreakSubsystem.h
// EMGStreakTier - defined in Streak/MGStreakSubsystem.h
// EMGPrestigeRank - defined in Prestige/MGPrestigeSubsystem.h
enum class EMGDamageState : uint8;  // Defined in Collision/MGCollisionSubsystem.h
// FMGCheckpointPassage - defined in Checkpoint/MGCheckpointSubsystem.h
// FMGLapData - defined in Checkpoint/MGCheckpointSubsystem.h
// FMGStuntEvent - defined in Stunt/MGStuntSubsystem.h (now included)
// EMGPowerupType - defined in Powerup/MGPowerupSubsystem.h (now included)
// EMGWeatherType - defined in Core/MGSharedTypes.h
// EMGCautionType - defined in Caution/MGCautionSubsystem.h
// EMGCautionReason - defined in Caution/MGCautionSubsystem.h
// FMGSafetyCarState - defined in Caution/MGCautionSubsystem.h
// EMGPenaltyType - defined in Penalty/MGPenaltySubsystem.h (now included)
// FMGPenalty - defined in Penalty/MGPenaltySubsystem.h (now included)
// EMGHeatLevel - defined in Core/MGSharedTypes.h
// FMGBountyCompletionResult - defined in Bounty/MGBountySubsystem.h
// FMGRaceEvent - defined in RaceDirector/MGRaceDirectorSubsystem.h (now included)
// EMGRaceEventType - defined in Race/MGRaceStarter.h (now included)
// EMGLicenseCategory - defined in License/MGLicenseSubsystem.h (now included)
// EMGLicenseTier - defined in License/MGLicenseSubsystem.h (now included)
// EMGTestGrade - defined in License/MGLicenseSubsystem.h (now included)
// FMGContract - defined in Contract/MGContractSubsystem.h
// FMGContractObjective - defined in Contract/MGContractSubsystem.h
// FMGChallenge - defined in Challenges/MGChallengeSubsystem.h
// EMGCurrencyType - defined in Core/MGSharedTypes.h
// FMGEarningMultiplier - defined in Currency/MGCurrencySubsystem.h (now included)
// FMGRewardClaimResult - defined in DailyRewards/MGDailyRewardsSubsystem.h (now included)
// EMGStreakMilestone - defined in DailyRewards/MGDailyRewardsSubsystem.h (now included)
// FMGDailyReward - defined in DailyRewards/MGDailyRewardsSubsystem.h (now included)
// EMGReputationCategory - defined in Reputation/MGReputationSubsystem.h (now included)
// EMGReputationTier - defined in Core/MGSharedTypes.h (via MGReputationSubsystem.h include)
// FMGReputationUnlock - defined in Reputation/MGReputationSubsystem.h (now included)
// FMGGhostComparator - defined in Ghost/MGGhostSubsystem.h (included below)
// EMGGhostComparison - defined in Ghost/MGGhostSubsystem.h (included below)
// EMGCareerChapter, EMGCareerMilestone, FMGCareerObjective - defined in Career/MGCareerSubsystem.h
// FMGRival - defined in Rivals/MGRivalsSubsystem.h (included below)
// EMGTimeOfDay - defined in Core/MGSharedTypes.h
// EMGDraftingZone - defined in Slipstream/MGSlipstreamSubsystem.h (now included)

// ============================================================================
// INPUT STATE STRUCT - Now defined in Input/MGVehicleInputHandler.h
// ============================================================================
// FMGVehicleInputState has been moved to Input/MGVehicleInputHandler.h
// to consolidate vehicle input types in the Input subsystem.
// 
// NOTE: Field mapping changes from legacy definition:
//   - bHandbrake (bool) -> Handbrake (float, 0-1)
//   - bNitro (bool) -> bNOSPressed (bool)
//   - bHorn (bool) -> bHornPressed (bool)
//   - GearShift (int32) -> bShiftUpRequested/bShiftDownRequested (bool)
//   - LookDirection (FVector) -> removed (use camera system instead)
//   + Clutch (float) - new
//   + LookLeft/LookRight (float) - new
//   + bCameraChange (bool) - new
//   + bHeadlightsToggle (bool) - new
// ============================================================================
...
public:
	...
	virtual void PlayerTick(float MGDeltaTime) override;
...
protected:
	...
	UFUNCTION()
	void OnSlipstreamEnteredByDistance(const FString& FollowerId, const FString& LeaderId, float Distance);

	/** Handle slingshot ready from aerodynamics subsystem */
	UFUNCTION()
	void OnSlingshotReadyWithParams(const FString& VehicleId, float BoostAmount, float Duration);
	...
	UFUNCTION()
	void OnCheckpointPassed(const FMGCheckpointPassage& Passage, int32 CheckpointsRemaining, float MGDeltaTime);
