...
// FMGEngineSoundLayer - MOVED TO EngineAudio/MGEngineAudioSubsystem.h (included above)
...
protected:
	...
	void UpdateLayers(float MGDeltaTime);
	...
	void UpdateExhaustPops(float MGDeltaTime);

	/** Create procedural engine sound */
	void UpdateProceduralSound();

	/** Calculate pitch based on current RPM and curve */
	float CalculatePitchInternal() const;
};
