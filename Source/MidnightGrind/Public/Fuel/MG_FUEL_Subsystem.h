...
// EMGFuelType - defined in Core/MGSharedTypes.h
 ...
 * This data is persisted across sessions for long-term tracking.
 */
USTRUCT(BlueprintType)
struct FMGFuelTelemetry
{
	...
	/// Rolling history of consumption readings (limited to last 600 samples)
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Telemetry")
	...
	/// Highest recorded consumption rate (liters/second)
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Telemetry")
	float PeakConsumption = 0.0f;

	/// Most efficient consumption rate achieved (liters/second)
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Telemetry")
	float LowestConsumption = 0.0f;

	/// Most fuel-efficient lap (least fuel used, in liters)
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Telemetry")
	float BestEfficiencyLap = 0.0f;

	/// Least efficient lap (most fuel used, in liters)
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Telemetry")
	float WorstEfficiencyLap = 0.0f;

	/// Total fuel used in current session (liters)
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Telemetry")
	float TotalFuelUsedSession = 0.0f;

	/// Total distance covered in session (meters)
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Telemetry")
	float TotalDistanceSession = 0.0f;

	/// Overall efficiency (meters per liter)
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Telemetry")
	float SessionEfficiency = 0.0f;

	// ==========================================
	// PERSISTENT LIFETIME STATS (Saved to disk)
	// ==========================================

	/// Total fuel consumed across all sessions (liters)
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Telemetry|Lifetime")
	float TotalFuelConsumed = 0.0f;

	/// Total distance covered across all sessions (meters)
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Telemetry|Lifetime")
	float TotalDistanceCovered = 0.0f;

	/// Best fuel economy achieved (meters per liter)
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Telemetry|Lifetime")
	float BestEconomy = 0.0f;

	/// Total number of refueling operations
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Telemetry|Lifetime")
	int32 TotalRefuels = 0;

	/// Total fuel added via refueling (liters)
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Telemetry|Lifetime")
...
public:
	...
	void UpdateFuelConsumption(FName VehicleID, float Throttle, float Speed, float RPM, int32 Gear, bool bNitroActive, float MGDeltaTime);
...
protected:
	...
	void UpdateRefueling(float MGDeltaTime);
