...
/**
 * @brief Wrapper for TArray<FMGPooledVFX> to support UPROPERTY in TMap.
 */
USTRUCT(BlueprintType)
struct FMGPooledVFXArray
{
	GENERATED_BODY()

	UPROPERTY(EditAnywhere, BlueprintReadWrite)
	TArray<FMGPooledVFX> Effects;
};
...
protected:
	...
	UPROPERTY()
	TMap<UNiagaraSystem*, FMGPooledVFXArray> VFXPool;
	...
	void UpdateScreenEffects(float MGDeltaTime);
