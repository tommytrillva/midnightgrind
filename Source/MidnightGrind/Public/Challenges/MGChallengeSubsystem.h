...
protected:
	...
	/// Player progress for each challenge (saved separately via SaveChallengeProgress)
	UPROPERTY(Transient)
	TMap<FName, FMGChallengeProgress> ChallengeProgress;

	/// IDs of currently active challenges (runtime tracking)
	UPROPERTY(Transient)
	...
	/// Active community challenges (synced from server)
	UPROPERTY(Transient)
	TArray<FMGCommunityChallenge> CommunityChallenges;

	/// Stat values for automatic objective tracking (session-only)
	UPROPERTY(Transient)
	TMap<FName, int32> TrackedStats;

	/// Maximum simultaneous active challenges (configurable)
	UPROPERTY(Config)
	int32 MaxActiveChallenges = 10;

	/// Remaining daily challenge rerolls (resets at daily reset)
	UPROPERTY(Transient)
