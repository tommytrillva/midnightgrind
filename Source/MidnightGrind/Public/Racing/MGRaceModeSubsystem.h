 ...
 * @brief Race Mode Subsystem - Core race logic, timing, and scoring management
 *
 ...
 * ============================================================================
 *
 * - Multi-race-type support (Circuit, Sprint, Drift, Drag, Elimination, etc.)
 * - Real-time position calculation based on race progress
 * - Precision timing for laps, sectors, and split times
 * - Drift scoring system for drift races
 * - Drag racing reaction time and distance splits
 * - Track record persistence and validation
 * - Performance class-based vehicle restrictions
 *
 * ============================================================================
 * RACE FLOW INTEGRATION
 * ============================================================================
 *
 * This subsystem is typically controlled by UMGRaceFlowSubsystem:
 * 1. RaceFlow calls SetupRace() with configuration
 * 2. RaceFlow calls AddRacer() for each participant
 * 3. RaceFlow calls StartCountdown() to begin
 * 4. Checkpoint triggers call RacerPassedCheckpoint()
 * 5. Race ends automatically or via FinishRace()
 *
 * @see UMGRaceFlowSubsystem for high-level race orchestration
 * @see UMGRaceDirectorSubsystem for AI pacing and rubber-banding
 * @see EMGRaceType for supported race types
 */
...
// Forward declarations - break circular dependency with GameModes
class AMGRaceGameMode;

// Forward declaration for vehicle pawn reference
...
// EMGRaceState - REMOVED (duplicate)
...
// EMGTrafficDensity - REMOVED (duplicate)
...
// FMGCheckpointData - MOVED TO Track/MGTrackSubsystem.h (included above)
// Note: Member names unified - use Index (not CheckpointIndex), Position (not Location),
//       bIsSectorSplit (not bIsSectorMarker). Added Width field.

// ============================================================================
// TRACK RECORD STRUCTURE
// ============================================================================

// FMGTrackRecord - MOVED TO Track/MGTrackDataAssets.h (included above)
...
/**
 * @brief Wrapper for TArray<float> to support UPROPERTY in TMap.
 */
USTRUCT(BlueprintType)
struct FMGRaceFloatArray
{
	GENERATED_BODY()
...
public:
	...
	void UpdateRacerTime(const FGuid& RacerID, float MGDeltaTime);
...
protected:
	...
	void UpdateEliminationTimer(float MGDeltaTime);
...
private:
	...
	UPROPERTY()
	TMap<FGuid, FMGRaceFloatArray> DragSplits;
