...
// EMGHeatLevel - REMOVED (duplicate)
// Canonical definition in: HeatLevel/MGHeatLevelSubsystem.h

/**
 * @brief Types of traffic violations that increase heat
 */
UENUM(BlueprintType)
enum class EMGPoliceViolationType : uint8
{
    None,            ///< No specific type (used as default/filter)
    Speeding,        ///< Excessive speed
    Reckless,        ///< Dangerous driving behavior
    RunRedLight,     ///< Running a red light
    HitCivilian,     ///< Collision with civilian
    HitPolice,       ///< Collision with police vehicle
    EvadePursuit,    ///< Fleeing active pursuit
    StreetRacing,    ///< Participating in illegal race
    PropertyDamage,  ///< Destroying property
    WrongWay,        ///< Driving against traffic
    Nitrous,         ///< Illegal use of nitrous
    RoadblockBreach, ///< Breaking through roadblock
    PoliceVehicleDestroyed, ///< Destroying police vehicle
    NearMissPolice,  ///< Near collision with police
    SpikeStripEvaded ///< Evading spike strip
};

// EMGPoliceUnitType - REMOVED (duplicate)
// Canonical definition in: HeatLevel/MGHeatLevelSubsystem.h
...
struct FMGViolationRecord
{
	...
	EMGPoliceViolationType Type = EMGPoliceViolationType::Speeding;
...
struct FMGCriminalRecord
{
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Record")
	TMap<EMGPoliceViolationType, int32> ViolationCounts;
...
public:
	...
	void AddHeat(int32 Amount, EMGPoliceViolationType Reason);
	...
	void ReportViolation(EMGPoliceViolationType Type, FVector Location);
	 ...
	 */
	UFUNCTION(BlueprintCallable, Category = "Police|Violations")
	void ReportViolationWithWitness(EMGPoliceViolationType Type, FVector Location, bool bWasWitnessed, int32 WitnessUnitID = -1);
	...
	int32 GetHeatForViolation(EMGPoliceViolationType Type) const;
	 ...
	 */
	UFUNCTION(BlueprintPure, Category = "Police|Violations")
	int64 GetFineForViolation(EMGPoliceViolationType Type) const;
...
protected:
	...
	void UpdatePursuit(float MGDeltaTime);

	/** Update cooldown timer */
	void UpdateCooldown(float MGDeltaTime);

	/** Update all police unit AI */
	void UpdatePoliceAI(float MGDeltaTime);

	/** Update bust progress if stopped near cops */
	void UpdateBustedState(float MGDeltaTime);
