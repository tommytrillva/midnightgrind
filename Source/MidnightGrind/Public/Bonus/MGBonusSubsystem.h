...
struct FMGBonusPlayerStats
{
	...
	/// Highest multiplier value ever achieved (also used for BestMultiplier)
	UPROPERTY(EditAnywhere, BlueprintReadWrite)
	...
	/// Longest bonus chain achieved
	UPROPERTY(EditAnywhere, BlueprintReadWrite)
	int32 LongestChain = 0;

	/// Best score achieved in a bonus round
	UPROPERTY(EditAnywhere, BlueprintReadWrite)
	int32 BestBonusRoundScore = 0;
};
...
/**
 * @brief Wrapper for TArray<FMGActiveBonus> to support UPROPERTY in TMap.
 */
USTRUCT(BlueprintType)
struct FMGActiveBonusArray
{
	GENERATED_BODY()

	UPROPERTY(EditAnywhere, BlueprintReadWrite)
	TArray<FMGActiveBonus> Bonuses;
};
...
public:
	...
	void UpdateBonusSystem(float MGDeltaTime);
...
protected:
	void TickBonus(float MGDeltaTime);
	void UpdateActiveBonuses(float MGDeltaTime);
	void UpdateSpawnRespawns(float MGDeltaTime);
	void UpdateBonusRounds(float MGDeltaTime);
...
private:
	...
	UPROPERTY()
	TMap<FString, FMGActiveBonusArray> PlayerActiveBonuses;
