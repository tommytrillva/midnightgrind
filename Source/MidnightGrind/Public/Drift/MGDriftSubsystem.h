...
/**
 * @brief Wrapper for TArray<FMGDriftLeaderboardEntry> to support UPROPERTY in TMap.
 */
USTRUCT(BlueprintType)
struct FMGDriftLeaderboardEntryArray
{
	GENERATED_BODY()

	UPROPERTY(EditAnywhere, BlueprintReadWrite)
	TArray<FMGDriftLeaderboardEntry> Entries;
};
...
public:
	...
	void UpdateDriftState(float MGDeltaTime, float SlipAngle, float Speed, const FVector& Position, const FVector& Velocity);
...
protected:
	...
	void CalculatePoints(float MGDeltaTime);
	...
	UPROPERTY()
	TMap<FName, FMGDriftLeaderboardEntryArray> ZoneLeaderboards;
