...
#include "Streak/MGStreakSubsystem.h" // FMGComboResult defined here
...
// FMGComboResult is defined in Streak/MGStreakSubsystem.h
...
/**
 * @brief Wrapper for TArray<FMGBufferedInput> to support UPROPERTY in TMap.
 */
USTRUCT(BlueprintType)
struct FMGBufferedInputArray
{
	GENERATED_BODY()

	UPROPERTY(EditAnywhere, BlueprintReadWrite)
	TArray<FMGBufferedInput> Inputs;
};
...
public:
    ...
    UFUNCTION(BlueprintCallable, Category = "InputBuffer|Core")
    void ProcessInputFrame(float MGDeltaTime);
...
protected:
    ...
    UPROPERTY()
    TMap<FName, FMGBufferedInputArray> ComboProgress;
    ...
    void UpdateActionStates(float MGDeltaTime);
    void CleanExpiredBuffers();
    void UpdateTimingWindows(float MGDeltaTime);
    void CheckForCombos();
    void CheckForDoubleTaps(EMGInputAction Action);
    void ProcessPlayback(float MGDeltaTime);
