...
/**
 * @brief Result of a single migration script execution.
 *
 * Contains detailed information about what happened during migration,
 * including timing, record counts, and any errors or warnings.
 */
USTRUCT(BlueprintType)
struct FMGMigrationResult
{
	...
	/** ID of the migration script that was executed */
	UPROPERTY(EditAnywhere, BlueprintReadWrite)
	FName ScriptID;
	...
	/** How long the migration took in seconds */
	UPROPERTY(EditAnywhere, BlueprintReadWrite)
	float DurationSeconds = 0.0f;

	/** Human-readable success message on completion */
	UPROPERTY(EditAnywhere, BlueprintReadWrite)
	FText Message;

	/** Error description if migration failed */
	UPROPERTY(EditAnywhere, BlueprintReadWrite)
	FString ErrorMessage;

	/** Number of data records successfully processed */
	UPROPERTY(EditAnywhere, BlueprintReadWrite)
