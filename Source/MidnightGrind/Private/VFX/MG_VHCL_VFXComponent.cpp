...
void UMGVehicleVFXComponent::TickComponent(float MGDeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
...
void UMGVehicleVFXComponent::UpdateTireSmoke(int32 WheelIndex, float MGDeltaTime)
{
	if (!TireStates.IsValidIndex(WheelIndex))
	{
		return;
...
void UMGVehicleVFXComponent::UpdateExhaustEffects(float MGDeltaTime)
{
	// Exhaust effects are updated via SetExhaustIntensity called from vehicle
}

void UMGVehicleVFXComponent::UpdateDamageEffects(float MGDeltaTime)
{
