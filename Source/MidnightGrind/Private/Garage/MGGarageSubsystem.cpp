...
FMGVehicleSpecs UMGGarageSubsystem::PreviewPartInstallation(const FGuid& VehicleId, const FMGPartData& Part) const
{
	FMGOwnedVehicle Vehicle;
	if (!GetVehicle(VehicleId, Vehicle))
	{
...
void UMGGarageSubsystem::RecalculateVehicleStats(const FGuid& VehicleId)
{
	...
	FMGVehicleSpecs CalculatedStats = UMGStatCalculator::CalculateAllStats(TempVehicleData, ModelData);
...
FMGVehicleSpecs UMGGarageSubsystem::GetVehicleStats(const FGuid& VehicleId) const
{
	const FMGVehicleSpecs* CachedStat = CachedStats.Find(VehicleId);
	if (CachedStat)
	{
		return *CachedStat;
	}
...
void UMGGarageSubsystem::ApplyCustomizationToVehicle(AMGVehiclePawn* Vehicle, const FGuid& VehicleId)
{
	...
	FMGVehicleSpecs Stats = GetVehicleStats(VehicleId);
...
bool UMGGarageSubsystem::ExportVehicleBuild(const FGuid& VehicleId, FString& OutJsonString) const
{
	...
	if (!GetVehicle(VehicleId, VehicleData))
	{
		return false;
	}
	const FMGOwnedVehicle* Vehicle = &VehicleData;
