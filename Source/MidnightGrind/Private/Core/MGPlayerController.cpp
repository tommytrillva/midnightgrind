...
void AMGPlayerController::BeginPlay()
{
	...
	if (IsLocalController())
	{
		...
		if (UGameInstance* GI = GetGameInstance())
		{
			...
			{
				AeroSubsystem->OnSlipstreamEntered.AddDynamic(this, &AMGPlayerController::OnSlipstreamEnteredByDistance);
				AeroSubsystem->OnSlingshotReady.AddDynamic(this, &AMGPlayerController::OnSlingshotReadyWithParams);
...
void AMGPlayerController::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
	...
	if (UGameInstance* GI = GetGameInstance())
	{
		...
		if (UMGAerodynamicsSubsystem* AeroSubsystem = GI->GetSubsystem<UMGAerodynamicsSubsystem>())
		{
			AeroSubsystem->OnSlipstreamEntered.RemoveDynamic(this, &AMGPlayerController::OnSlipstreamEnteredByDistance);
			AeroSubsystem->OnSlingshotReady.RemoveDynamic(this, &AMGPlayerController::OnSlingshotReadyWithParams);
...
void AMGPlayerController::PlayerTick(float MGDeltaTime)
{
...
void AMGPlayerController::OnSlipstreamEnteredByDistance(const FString& FollowerId, const FString& LeaderId, float Distance)
{
	// Only show for local player
	FString LocalPlayerId = GetLocalPlayerId();
...
void AMGPlayerController::OnSlingshotReadyWithParams(const FString& VehicleId, float BoostAmount, float Duration)
{
...
void AMGPlayerController::OnCheckpointPassed(const FMGCheckpointPassage& Passage, int32 CheckpointsRemaining, float MGDeltaTime)
{
	if (UWorld* World = GetWorld())
	{
		if (UMGRaceHUDSubsystem* HUDSubsystem = World->GetSubsystem<UMGRaceHUDSubsystem>())
