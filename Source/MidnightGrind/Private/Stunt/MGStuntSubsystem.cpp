...
void UMGStuntSubsystem::UpdateAirState(FVector CurrentPosition, FRotator CurrentRotation, float MGDeltaTime)
{
...
void UMGStuntSubsystem::UpdateTwoWheelDriving(float Distance, float TiltAngle, float MGDeltaTime)
{
	...
	if (UGameInstance* GI = GetGameInstance())
	{
		if (UMGSaveManagerSubsystem* SaveManager = GI->GetSubsystem<UMGSaveManagerSubsystem>())
		{
			if (const UMGSaveGame* SaveData = SaveManager->GetCurrentSaveData())
			{
				...
				SessionStats.TotalAirTime = SaveData->StuntData.HighestAirTime;
...
void UMGStuntSubsystem::TickCombo(float MGDeltaTime)
{
	if (CurrentCombo.ComboCount == 0)
	{
		return;
