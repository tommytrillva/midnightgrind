...
FMGVehicleSpecs UMGTuningSubsystem::GetBaseStats(FName VehicleID) const
{
	if (const FMGVehicleSpecs* Stats = BaseVehicleStats.Find(VehicleID))
	{
		return *Stats;
	}

	return FMGVehicleSpecs();
}

FMGVehicleSpecs UMGTuningSubsystem::GetTunedStats(FName VehicleID) const
{
	if (const FMGVehicleSpecs* Stats = TunedVehicleStats.Find(VehicleID))
	{
		return *Stats;
	}
...
{
	FMGVehicleSpecs Stats = GetTunedStats(VehicleID);
	return Stats.PerformanceIndex;
}

FMGVehicleSpecs UMGTuningSubsystem::PreviewPartInstall(FName VehicleID, FName PartID) const
{
	FMGVehicleSpecs BaseStats = GetBaseStats(VehicleID);
	FMGVehicleSpecs PreviewStats = GetTunedStats(VehicleID);
...
void UMGTuningSubsystem::RegisterVehicle(FName VehicleID, const FMGVehicleSpecs& BaseStats)
{
...
FMGVehicleSpecs UMGTuningSubsystem::CalculateTunedStats(FName VehicleID) const
{
	FMGVehicleSpecs Stats = GetBaseStats(VehicleID);
...
{
	FMGVehicleSpecs NewStats = CalculateTunedStats(VehicleID);
