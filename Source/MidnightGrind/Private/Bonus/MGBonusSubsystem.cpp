...
void UMGBonusSubsystem::UpdateBonusSystem(float MGDeltaTime)
{
...
void UMGBonusSubsystem::SaveBonusData()
{
	...
	for (const auto& Pair : PlayerStats)
	{
		...
		Archive << Stats.HighestMultiplierValue;
		Archive << Stats.LongestChain;
		Archive << Stats.SecretBonusesFound;
		Archive << Stats.BonusRoundsCompleted;
		...
		// Write type counts
		int32 TypeCount = Stats.BonusesCollected.Num();
		Archive << TypeCount;
		for (const auto& TypePair : Stats.BonusesCollected)
		{
...
void UMGBonusSubsystem::LoadBonusData()
{
	...
	for (int32 i = 0; i < PlayerCount; i++)
	{
		...
		for (int32 j = 0; j < TypeCount; j++)
		{
			...
			Archive << TypeInt;
			Archive << Count;
			Stats.BonusesCollected.Add(static_cast<EMGBonusType>(TypeInt), Count);
		}
...
void UMGBonusSubsystem::TickBonus(float MGDeltaTime)
{
	UpdateBonusSystem(DeltaTime);
}

void UMGBonusSubsystem::UpdateActiveBonuses(float MGDeltaTime)
{
...
void UMGBonusSubsystem::UpdateSpawnRespawns(float MGDeltaTime)
{
	for (auto& Pair : SpawnPoints)
	{
...
void UMGBonusSubsystem::UpdateBonusRounds(float MGDeltaTime)
{
