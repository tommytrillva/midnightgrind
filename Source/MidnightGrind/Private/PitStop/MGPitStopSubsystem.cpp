...
/**
 * @file MGPitStopSubsystem.cpp
 * @brief Implementation of the Pit Stop Management Subsystem.
 *
 * Handles all pit stop operations including vehicle servicing workflow,
 * pit lane traffic management, crew simulation with skill/fatigue systems,
 * tire compound selection, and race strategy planning/optimization.
 *
 * @see UMGPitStopSubsystem
 */
...
void UMGPitStopSubsystem::UpdateActivePitStops(float MGDeltaTime)
{
...
void UMGPitStopSubsystem::ProcessService(FMGActivePitStop& PitStop, float MGDeltaTime)
{
	if (PitStop.PendingServices.Num() == 0)
	{
		return;
