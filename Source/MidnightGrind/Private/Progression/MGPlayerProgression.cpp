...
bool UMGPlayerProgression::MeetsUnlockRequirements(const FMGUnlockRequirement& Requirement) const
{
	...
	if (Requirement.RequiredLevel > 0 && LevelProgression.CurrentLevel < Requirement.RequiredLevel)
	{
		return false;
	}

	// Check crew reputation (RequiredCrewID maps to EMGCrew enum value)
	if (Requirement.RequiredCrewID > 0 && Requirement.RequiredCrewReputation > 0)
	{
		EMGCrew RequiredCrew = static_cast<EMGCrew>(Requirement.RequiredCrewID);
		if (GetCrewReputation(RequiredCrew) < Requirement.RequiredCrewReputation)
		{
			return false;
		}
	}

	// Check race wins (supports both RequiredWins and RequiredRaceWins for flexibility)
	int32 WinsNeeded = FMath::Max(Requirement.RequiredWins, Requirement.RequiredRaceWins);
	if (WinsNeeded > 0 && Statistics.TotalWins < WinsNeeded)
	{
		return false;
	}

	// Check total races completed
	if (Requirement.RequiredRaces > 0 && Statistics.TotalRaces < Requirement.RequiredRaces)
	{
		return false;
	}

	// Check prerequisite unlocks (array version)
	...
	// Check single prerequisite unlock
	if (!Requirement.RequiredPreviousUnlock.IsNone() && !IsUnlocked(Requirement.RequiredPreviousUnlock))
	{
		return false;
	}
