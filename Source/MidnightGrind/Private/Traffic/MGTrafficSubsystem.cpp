...
// Internal
void UMGTrafficSubsystem::UpdateTraffic(float MGDeltaTime)
{
...
void UMGTrafficSubsystem::UpdateVehicleAI(FMGTrafficVehicleState& Vehicle, float MGDeltaTime)
{
...
void UMGTrafficSubsystem::UpdateTrafficLights(float MGDeltaTime)
{
	if (bForcedGreenLights)
	{
		return;
...
int32 UMGTrafficSubsystem::SpawnEmergencyVehicle(FVector Location, FRotator Rotation, EMGTrafficVehicleType Type, bool bActivateSirens)
{
	// Validate emergency vehicle type
	if (!IsEmergencyVehicle(Type))
	{
		return -1;
	}

	int32 VehicleID = SpawnTrafficVehicle(Location, Rotation, Type);
	if (VehicleID != -1 && bActivateSirens)
	{
		SetEmergencySirens(VehicleID, true);
	}

	return VehicleID;
}

int32 UMGTrafficSubsystem::GetEmergencyVehicleCount() const
{
	int32 Count = 0;
	for (const auto& Pair : ActiveVehicles)
	{
		if (IsEmergencyVehicle(Pair.Value.VehicleType))
		{
			Count++;
		}
	}
...
bool UMGTrafficSubsystem::IsEmergencyVehicle(EMGTrafficVehicleType VehicleType)
{
	return VehicleType == EMGTrafficVehicleType::Ambulance ||
		   VehicleType == EMGTrafficVehicleType::FireTruck ||
		   VehicleType == EMGTrafficVehicleType::PoliceCar;
}

void UMGTrafficSubsystem::MakeVehicleHonk(int32 VehicleID, float Duration)
{
	FMGTrafficVehicleState* Vehicle = ActiveVehicles.Find(VehicleID);
	if (Vehicle)
	{
		Vehicle->bIsHonking = true;
		Vehicle->HonkTimeRemaining = Duration;
		OnTrafficHonk.Broadcast(VehicleID, Vehicle->Location);
	}
}

void UMGTrafficSubsystem::SetEmergencySirens(int32 VehicleID, bool bActivate)
{
	FMGTrafficVehicleState* Vehicle = ActiveVehicles.Find(VehicleID);
	if (Vehicle && IsEmergencyVehicle(Vehicle->VehicleType))
	{
		Vehicle->bSirensActive = bActivate;
		
		// Emergency vehicles with active sirens use emergency response behavior
		if (bActivate)
		{
			Vehicle->Behavior = EMGTrafficBehavior::EmergencyResponse;
			Vehicle->TargetSpeed = 60.0f; // Emergency vehicles drive faster
		}
		else
		{
			Vehicle->Behavior = EMGTrafficBehavior::Normal;
			Vehicle->TargetSpeed = 35.0f;
		}
	}
}

// ============================================================================
// Player Interaction (Extended)
// ============================================================================

void UMGTrafficSubsystem::UpdatePlayerState(FVector Position, FVector Velocity, float Speed, bool bIsDrifting)
{
	PlayerPosition = Position;
	PlayerVelocity = Velocity;
	PlayerSpeed = Speed;
	bPlayerIsDrifting = bIsDrifting;
...
float UMGTrafficSubsystem::GetEffectiveDensityMultiplier() const
{
	float BaseDensity = GetDensityMultiplier();

	// Apply time-of-day scaling if subsystem is available
	if (TimeOfDaySubsystem.IsValid())
	{
		// This would query the time subsystem for rush hour multipliers, etc.
		// For now, return base density
		return BaseDensity;
	}
...
void UMGTrafficSubsystem::RefreshTimeOfDaySettings()
{
	// Cache reference to time of day subsystem
	if (UWorld* World = GetWorld())
	{
		TimeOfDaySubsystem = World->GetSubsystem<UMGTimeOfDaySubsystem>();
	}
}
...
TArray<FMGTrafficNearMissEvent> UMGTrafficSubsystem::GetRecentNearMisses(int32 MaxCount) const
{
	TArray<FMGTrafficNearMissEvent> Result;
	int32 Count = FMath::Min(MaxCount, RecentNearMisses.Num());
	for (int32 i = 0; i < Count; i++)
	{
		Result.Add(RecentNearMisses[i]);
	}
	return Result;
}
