...
bool UMGEconomySubsystem::ClaimDailyLoginBonus(int64& OutBonusAmount)
{
	...
	// Base bonus: $500, increases by $100 per day up to 7-day max
	constexpr int64 BaseDailyBonus = 500;
	constexpr int64 BonusPerStreakDay = 100;
	constexpr int32 MaxStreakBonus = 7;

	const int32 StreakBonusDays = FMath::Min(LoginStreakDays, MaxStreakBonus);
	OutBonusAmount = BaseDailyBonus + (StreakBonusDays * BonusPerStreakDay);

	// Update streak
	const FDateTime Today = FDateTime::Today();
	const FDateTime LastLoginDay = FDateTime(LastLoginDate.GetYear(), LastLoginDate.GetMonth(), LastLoginDate.GetDay());

	if ((Today - LastLoginDay).GetDays() == 1)
	{
		// Consecutive day - increase streak
		LoginStreakDays++;
	}
	else if ((Today - LastLoginDay).GetDays() > 1)
	{
		// Streak broken - reset
		LoginStreakDays = 1;
	}
	...
	// Award the bonus
	AddCredits(OutBonusAmount, EMGTransactionType::DailyBonus,
		FText::Format(NSLOCTEXT("Economy", "DailyBonus", "Daily Login Bonus (Day {0})"), FText::AsNumber(LoginStreakDays)));

	return true;
}

bool UMGEconomySubsystem::IsDailyBonusAvailable() const
{
	if (!LastLoginDate.GetTicks())
	{
		// Never logged in before - bonus available
		return true;
	}
	...
	// Bonus available if last claim was before today
...
bool UMGEconomySubsystem::DeductCash(FGuid PlayerID, int64 Amount, const FString& Description)
{
	// PlayerID unused in single-player - for future multiplayer support
	return SpendCredits(Amount, EMGTransactionType::MarketplacePurchase, FText::FromString(Description));
}

void UMGEconomySubsystem::AddCash(FGuid PlayerID, int64 Amount, const FString& Description)
{
	// PlayerID unused in single-player - for future multiplayer support
	AddCredits(Amount, EMGTransactionType::MarketplaceSale, FText::FromString(Description));
}
...
FText UMGEconomySubsystem::GetTransactionTypeName(EMGTransactionType Type)
{
	switch (Type)
	{
		...
		case EMGTransactionType::ToolPurchase:	return NSLOCTEXT("Transaction", "ToolPurchase", "Tool Purchase");
		case EMGTransactionType::GarageUpgrade:	return NSLOCTEXT("Transaction", "GarageUpgrade", "Garage Upgrade");
		case EMGTransactionType::FuelPurchase:	return NSLOCTEXT("Transaction", "FuelPurchase", "Fuel Purchase");
