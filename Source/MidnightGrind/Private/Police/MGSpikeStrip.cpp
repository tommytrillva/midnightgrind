 ...
 * to slow down fleeing vehicles by damaging their tires. Features
 * configurable length, single-use/reusable modes, and progressive
 * damage based on vehicle speed.
 *
 * @see AMGSpikeStrip
 */
...
void AMGSpikeStrip::Tick(float MGDeltaTime)
{
...
void AMGSpikeStrip::UpdateDeployAnimation(float MGDeltaTime)
{
...
void AMGSpikeStrip::OnTriggerBeginOverlap(UPrimitiveComponent* OverlappedComponent, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepResult)
{
	...
	// Convert from cm/s to MPH: (cm/s) * (1 m / 100 cm) * (1 mile / 1609.34 m) * (3600 s / 1 hr)
	static constexpr float CmPerSecToMPH = 0.0223694f;
	float VehicleSpeedMPH = VehicleSpeed * CmPerSecToMPH;
