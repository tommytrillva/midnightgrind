...
/**
 * @file MGSubsystemTests.cpp
 * @brief Implementation of comprehensive subsystem test suite.
 *
 * Contains 70+ automated tests covering all core game subsystems:
 * Currency, Weather, Economy, Vehicle, AI, Performance, Save/Load,
 * Physics, Stress, UI Data, Menu, Notification, and Race Flow.
 *
 * Run tests via console commands (MG.RunAllTests) or Blueprint calls.
 */
...
#include "AI/MG_AI_RacerSubsystem.h"
...
FMGTestResult UMGSubsystemTests::TestPerf_MemoryUsage()
{
	...
	// Test allocation/deallocation
	{
		...
		{
			void* Ptr = FMemory::MallocQuantized(AllocSize);
			if (Ptr)
			{
				Allocations.Add(Ptr);
			}
