...
/**
 * @file MGRaceModeSubsystem.cpp
 * @brief Implementation of the Race Mode Subsystem
 *
 * Contains the core race logic implementation including:
 * - Race state machine management
 * - Position calculation and tracking
 * - Timing systems (lap times, sector times, splits)
 * - Drift scoring mechanics
 * - Drag racing reaction time and splits
 * - Track record management
 * - Race result generation
 *
 * @see MGRaceModeSubsystem.h for class documentation
 */
...
void UMGRaceModeSubsystem::UpdateRacerTime(const FGuid& RacerID, float MGDeltaTime)
{
	for (FMGRacerEntry& Racer : Racers)
	{
...
void UMGRaceModeSubsystem::UpdateEliminationTimer(float MGDeltaTime)
{
