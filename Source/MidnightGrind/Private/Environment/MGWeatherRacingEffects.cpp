...
void UMGWeatherRacingSubsystem::Tick(float MGDeltaTime)
{
...
void UMGWeatherRacingSubsystem::UpdateAIForWeather(AMGRacingAIController* AIController)
{
	...
	FMGAIDriverConfig Profile = AIController->GetDriverProfile();
...
void UMGWeatherRacingSubsystem::UpdatePuddles(float MGDeltaTime)
{
...
void UMGWeatherRacingSubsystem::UpdateWindGusts(float MGDeltaTime)
{
	FMGWindState& Wind = CurrentEffects.WindState;
...
void UMGWeatherRacingSubsystem::SpawnRainPuddles(float MGDeltaTime)
{
	if (!WeatherSubsystem.IsValid())
	{
		return;
