...
void UMGAIRaceManager::TickComponent(float MGDeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
...
int32 UMGAIRaceManager::SpawnAIOpponent(const FMGAIOpponentConfig& Config, const FTransform& SpawnTransform)
{
	...
	if (!SpawnedVehicle)
	{
		UE_LOG(LogTemp, Warning, TEXT("MG_AI_RaceManager: Failed to spawn AI vehicle"));
		return -1;
	}
	...
	if (!AIController)
	{
		...
		UE_LOG(LogTemp, Warning, TEXT("MG_AI_RaceManager: Failed to spawn AI controller"));
		return -1;
	}
	...
	UE_LOG(LogTemp, Log, TEXT("MG_AI_RaceManager: Spawned AI opponent %d (%s)"),
...
TArray<int32> UMGAIRaceManager::GenerateAIField(int32 OpponentCount, EMGAIDifficulty BaseDifficulty, const TArray<FTransform>& SpawnTransforms)
{
	...
	for (int32 i = 0; i < OpponentCount; ++i)
	{
		...
		Config.DriverProfile = FMGAIDriverConfig();
...
void UMGAIRaceManager::StartRacing()
{
	...
	UE_LOG(LogTemp, Log, TEXT("MG_AI_RaceManager: Started racing with %d AI opponents"), ActiveOpponents.Num());
}
